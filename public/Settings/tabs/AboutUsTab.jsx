...
export const AboutUsTab = ({ onOpenAgreement, onOpenPrivacy }) => {
  ...
  const websiteUrl = 'https://joblink-clean-deploy.netlify.app';
  ...
  const handleCopyWebsite = async () => {
    try {
      ...
      if (Capacitor.isNativePlatform() && typeof Clipboard?.write === 'function') {
        try {
          await Clipboard.write({ string: websiteUrl });
          copied = true;
        } catch (nativeError) {
          console.warn('[AboutUsTab] Capacitor clipboard write failed, trying browser fallback:', nativeError);
        }
      }
  ...
  return (
    <div className="about-us-tab">
      <div className="about-us-logo-wrapper">
        <img className="about-us-logo" src={LinkWhite} alt="Joblink logo" />
